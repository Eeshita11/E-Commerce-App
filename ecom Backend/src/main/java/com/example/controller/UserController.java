...
public class UserController 
{
   ...
   @GetMapping(value="/showallusers")
    ...
    @DeleteMapping(value = "/deletebyregid/{reg_id}")
    public ResponseEntity<String> deleteRegisterById(@PathVariable Integer reg_id) 
    {
        Optional<Register> register = userService.getregisterbyid(reg_id);
        if (register.isPresent()) 
        {
            userService.deleteRegisterById(reg_id);
            return ResponseEntity.ok("Register with ID " + reg_id + " deleted successfully.");
        } 
       else 
        {
            return ResponseEntity.status(HttpStatus.NOT_FOUND).body("Register with ID " + reg_id + " not found.");
        }
    }
